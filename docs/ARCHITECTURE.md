...
Protected `main`. Operator `GET /api/v1/repos/code/voting/branch_protections` for the `main` rule. **Attested** GET `updated_at` 2026-09-21T07:29:30Z (after [cl8y-forgejo#48](https://git.cl8y.com/PlasticDigits/cl8y-forgejo/issues/48) rollout on this repo). Unauthenticated GET returns **401** (`token is required`); successor close still **re-GETs**. Flag *values* match the canary table in `code/hello` architecture **H15**; IDs here are **H30-*** because the original ticket is [#30](https://git.cl8y.com/code/voting/issues/30).

This tree does not PATCH Forgejo protection JSON, change CAC autoland predicates, Coolify `vote.cl8y.com` hosting, Legal admin, or product runtime. Those remain [cl8y-forgejo#48](https://git.cl8y.com/PlasticDigits/cl8y-forgejo/issues/48), [cl8y-agent-control#429](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/429), and [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297). Do not add a local `docs/INVARIANTS.md`; the original issue body points at [cl8y-forgejo’s copy](https://git.cl8y.com/PlasticDigits/cl8y-forgejo/src/branch/main/docs/INVARIANTS.md).

A green Woodpecker `find` / `tree`-named clone-check, or a green GitLab `test:rust` / `lint:gitleaks` job, is **not** proof of **H30-2**, **H30-3**, **H30-5**, **H30-8**, or **H30-9**. Remaining-work successor must not PATCH protection. Re-read at successor merge; fail on drift from the table below. Merge of [#30](https://git.cl8y.com/code/voting/pulls/30) with empty statuses does not waive **H30-3**.
...
| **H30-8** | `block_on_official_review_requests` | `false` (leftover official requests on merged [#30](https://git.cl8y.com/code/voting/pulls/30) / open [#29](https://git.cl8y.com/code/voting/pulls/29) must not block) |
...
| --- | --- | --- |
| `block_on_outdated_branch` | `true` | Do not PATCH from remaining-work. Successor head must rebase onto current `main` before merge. |
| `dismiss_stale_approvals` | `true` | Do not PATCH from remaining-work. |
| `apply_to_admins` | `false` | Do not PATCH from remaining-work. |
...
| ID | Rule |
| --- | --- |
| **H30-1** | `test ! -e` is true on `CODEOWNERS`, `docs/CODEOWNERS`, `.gitea/CODEOWNERS`, and `.forgejo/CODEOWNERS`. Absence, including empty or comments-only files (Forgejo still parses those). Not “the file exists but is not requesting reviewers.” File line `@code/maintainers` and PR JSON team `name` `maintainers` (org `code`, team id 4) are the same leftover plant. This tree has no vendor `CODEOWNERS`. Already true on current `origin/main`; successor must not restore (do not use `cac-design-issue-30` as the product tip). |
| **H30-4** | Merge is SHA-pinned `Do: merge` (`head_commit_id`). Never document or use `force_merge`. |
| **H30-6** | No Coolify/Woodpecker secrets on `pull_request`. This tree does not grow a `vote.cl8y.com` deploy, Legal admin, or ledger-writer step from remaining-work. Hello `telegram-failure` is forbidden. Coolify / Legal ops stay [#7](https://gitlab.com/PlasticDigits/voting/-/issues/7) / #297. |
| **H30-7** | This tree does not expand CAC merge/deploy/spend/custody policy. |

Official CODEOWNERS review is **not** a merge gate. Forgejo loads the first existing file among `CODEOWNERS`, `docs/CODEOWNERS`, `.gitea/CODEOWNERS`, and `.forgejo/CODEOWNERS` (Go-regexp, not GitHub globs). A catch-all `.*` still plants team `maintainers` on every non-WIP PR that changes a matching path. After ADR 0001, `test ! -e` is true on all four (**H30-1**). None of those paths may contain a reviewer rule for any pattern.

Root README must point at this gate and ADR 0001 (product tables stay; add the Decision 3 paragraph on the **successor** PR). Architecture-only is not a substitute for that pointer. Do not merge transport ref `cac-design-issue-30`; fork current `origin/main` and **copy** the two standing docs files.
...
`status_check_contexts` stays `ci/woodpecker/pr/woodpecker` (**H30-3**). Current `origin/main` (after merge `1d85943` of [#30](https://git.cl8y.com/code/voting/pulls/30)) still has **no** root `.woodpecker.yaml`. Commit statuses on `eb352b8` and merge `1d85943` are **empty** (`[]`). Merge of `#30` with empty statuses does **not** waive **WP-pre** or **H30-3**. Empty statuses with no YAML are ambiguous: missing file **or** repo not registered as a Woodpecker project. A posting **success** `ci/woodpecker/pr/woodpecker` from Woodpecker (`ci.cl8y.com`) on the **successor** head is a **land prerequisite** for ADR 0001 (**WP-pre**), not leftover and not a reason to drop **H30-3** or `force_merge`.
...
**#30 land bootstrap** (not this repo’s product test gate). Remaining-work implement MUST add exactly this file on the **successor** PR. Cargo tests, frontend `npm test` / Playwright, SPA fallback, and GitLab gitleaks stay **ungated** by this ticket. Any later real Forgejo CI **must extend this same root file**. A leftover to extend this same root file is optional and is **not** a close gate for remaining-work. The always-green `find` clone-check is the accepted org stub. Do not expand remaining-work into cargo / frontend / gitleaks on Woodpecker.
...
`when:` is two list items: `push`/`manual` on `main`, plus **unfiltered** `pull_request`. Keep that unfiltered `pull_request` item; it is the org stub. `branch: main` on a `pull_request` event filters the **target** branch, not the head (a PR whose target is `main` would still match). Do not add that filter anyway. `steps:` syntax, not `pipeline:`. Image is the hello/dex digest pin, not a floating tag. Command is the Token-SC / OTC / BASE_Buster / cmm `find` one-liner. Stock Alpine has no `tree` binary; do not `apk add tree`. Step name `tree` is the org stub name; the command is still `find`. No secrets, Coolify, Telegram, BSC RPC, or Terra LCD. Do not copy hello’s `coolify-deploy` / `telegram-failure`. Do not copy dex’s `scripts/ci/gitleaks-scan-tracked.sh` (that script does not exist here). Do not port `.gitlab-ci.yml` `test:rust` / `test:frontend` / `lint:gitleaks` onto the successor.

**WP-enable (pre-merge gate, before pushing the successor).** Confirm `code/voting` is an active Woodpecker project (Forgejo repo id **43** on `ci.cl8y.com`; webhook to `https://ci.cl8y.com/api/hook` for `push` + `pull_request`). Pass/fail GET (optional extra proof; still a land blocker if it fails): Forgejo `GET /api/v1/repos/code/voting/hooks` shows an **active** hook whose URL is `https://ci.cl8y.com/api/hook` and whose events include `push` **and** `pull_request`, **or** Woodpecker repo GET for `code/voting` has `active == true`. Fail closed on missing hook, wrong URL, missing events, or `active == false`. **WP-pre** (success status from `ci.cl8y.com` on the successor) remains the land proof; this GET does not replace it. File owner (remaining-work implement) ≠ posting owner (forge ops). dex#1247 recorded 44/44 `code/*` enablement on 2026-09-12; that historical count is **not** this preflight. Empty statuses on merged `#30` are **not** this preflight and do not waive **WP-pre** / **H30-3**. If the quoted YAML is on the successor and nothing posts: **land blocked** — open or wait on a **named** forge unblock ([cl8y-forgejo#48](https://git.cl8y.com/PlasticDigits/cl8y-forgejo/issues/48) or a local leftover shaped like [dex#1247](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1247)) **plus retrigger**. Do not POST a fake commit status. Do not `force_merge`. Do not drop **H30-3**. Owners: ADR 0001 Decision 8. Do not push onto closed `pulls/30`. Slice S3 must not fire just because `#30` merged.
