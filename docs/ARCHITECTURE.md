...
| ID | Rule |
| --- | --- |
| **H30-1** | `test ! -e` is true on `CODEOWNERS`, `docs/CODEOWNERS`, `.gitea/CODEOWNERS`, and `.forgejo/CODEOWNERS`. Absence, including empty or comments-only files (Forgejo still parses those). Not “the file exists but is not requesting reviewers.” File line `@code/maintainers` and PR JSON team `name` `maintainers` (org `code`, team id 4) are the same leftover plant. This tree has no vendor `CODEOWNERS`. |
...
**#30 land bootstrap** (not this repo’s product test gate). Implement MUST add exactly this file. Cargo tests, frontend `npm test` / Playwright, SPA fallback, and GitLab gitleaks stay **ungated** by this ticket. Any later real Forgejo CI **must extend this same root file**. A leftover to extend this same root file is optional and is **not** a close gate for #30. The always-green `find` clone-check is the accepted org stub. Do not expand #30 into cargo / frontend / gitleaks on Woodpecker.
