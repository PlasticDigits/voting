...
Production `DATABASE_URL` must be the restricted role from [`../deploy/grants.sql`](../deploy/grants.sql) — never the ledger writer. `RUN_MODE=prod` skips sqlx migrations (`APPLY_MIGRATIONS=false`) and requires CORS plus a non-zero POST rate limit. Invariants: [`../docs/OPERATOR_VOTING.md`](../docs/OPERATOR_VOTING.md) O-RL · [`../docs/OPS.md`](../docs/OPS.md).
